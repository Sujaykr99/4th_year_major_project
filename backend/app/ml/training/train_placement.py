...
class PlacementDataProcessor:
    ...
    def transform_input(self, input_data: Dict[str, Any]) -> np.ndarray:
        ...
        # Explicitly handle known categorical columns (must be strings for label encoding)
        categorical_cols = ['gender', 'branch', 'college_tier', 'volunteer_experience']
        for col in categorical_cols:
            if col in df.columns and col in self.label_encoders:
                df[col] = df[col].astype(str)

        # Now identify numeric cols (after categorical are converted to object)
        numeric_cols = df.select_dtypes(include=[np.number]).columns.tolist()
        categorical_cols_actual = df.select_dtypes(include=["object"]).columns.tolist()

        # Apply encoding to categorical columns
        for col in categorical_cols_actual:
            if col in self.label_encoders:
                le = self.label_encoders[col]
        ...
        # Apply scaling to numeric columns only (scaler was fit on numeric cols only)
        if numeric_cols:
